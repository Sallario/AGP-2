...
float _scaleUpdate = 1.0f;
float _redValue = 1.0f;
float _degrees = 45.0f;
...
char g_TutorialName[100] = "Alister Hartwell Tutorial 12\0";
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_UP)
		{
			_scaleUpdate += -0.1f;
			_redValue += -0.1f;
			g_Model3->IncScale(0.2f);
			g_Model4->IncScale(0.2f);
		}
		...
		{
			_degrees += 2.0f;
			g_Model3->IncXRot(2.0f);
			g_Model3->IncYRot(2.0f);
			g_Model3->IncZRot(2.0f);
		...
		{
			g_Model->MoveForward(0.01f);
			g_Model2->MoveForward(0.01f);

			if (g_Model->CheckCollision(g_Model2) == true)
			{
				g_Model->MoveForward(-0.3);
				g_Model2->MoveForward(-0.3);
			}
		}
		if (wParam == 0x52) // R key
		{
			g_Model->MoveForward(-0.1f);
			g_Model2->MoveForward(-0.1f);
		}
		//if (wParam == VK_PRIOR)// Page up key
		//{
		//	mainCamera->LookUpward(true);
		//}
		//if (wParam == VK_NEXT) // Page down key
		//{
		//	mainCamera->LookDownward(true);
		//}
...
HRESULT InitialiseGraphics() //03-01
{
	...
	g_Model->LoadObjModel("assets/PointySphere.obj");

	g_Model2 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_Model2->LoadObjModel("assets/PointySphere.obj");

	g_Model3 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_Model3->LoadObjModel("assets/cube.obj");

	g_Model4 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_Model4->LoadObjModel("assets/Sphere.obj");
	...
	g_Model4->SetScale(0.7);


	//g_Model4->LookAt_XZ(g_Model3->GetXPos(), g_Model3->GetZPos());


	// Different scales since apparently it needs to be this way
	g_Model->SetScale(0.6f);
	g_Model2->SetScale(0.5f);
	...
	//Set up camera
	mainCamera = new Camera(0.0f, 0.0f, -0.5f, 0.0f);
	/*mainCamera = new Camera();
	mainCamera->SetPosition(0.0f, 0.0f, 0.5f);
...
void RenderFrame(void)
{
	...
	g_Model2->SetLighting(g_directional_light_shines_from, g_directional_light_colour, g_ambient_light_colour);
	g_Model3->SetLighting(g_directional_light_shines_from, g_directional_light_colour, g_ambient_light_colour);
	g_Model4->SetLighting(g_directional_light_shines_from, g_directional_light_colour, g_ambient_light_colour);
	...
	world = XMMatrixRotationX(XMConvertToRadians(_degrees));
	world *= XMMatrixRotationY(XMConvertToRadians(_degrees));
	world *= XMMatrixRotationZ(XMConvertToRadians(_degrees));
	...
	// Make models look at each other
	g_Model->LookAt_XZ(g_Model2->GetXPos(), g_Model2->GetZPos());
	g_Model2->LookAt_XZ(g_Model->GetXPos(), g_Model->GetZPos());

	g_Model4->SetYRot(mouseX);
	g_Model4->SetXRot(mouseY);
	...
	g_Model2->Draw(&view, &projection);
	g_Model3->Draw(&view, &projection);
	g_Model4->Draw(&view, &projection);
